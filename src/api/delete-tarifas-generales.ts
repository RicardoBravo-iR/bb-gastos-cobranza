 ...
 */
export async function deleteTarifaGeneral(tarifa_id: string): Promise<void> {
  try {
    const token = await getCognitoToken();
    const url = `${API_SAC}cobranzas/rateCharge/delete`;
    console.log("[DELETE] Token usado:", token);
    console.log("[DELETE] Payload de eliminación: ", { tarifa_id });
